...
/* Private variables ---------------------------------------------------------*/
static const uint16_t SizeLed_C = 512;
static const uint16_t SizeSwitch_C = 512;
...
/* USER CODE BEGIN PF */
/**
 * @brief  Characteristic update extended
 * @param  ConnectionHandle: BLE connection handle
 * @param  CharOpcode: Characteristic identifier
 * @param  Service_Instance: Instance of the service to which the characteristic belongs
 *
 */
tBleStatus P2P_SERVER_UpdateValue_Ext(uint16_t ConnectionHandle, P2P_SERVER_CharOpcode_t CharOpcode, P2P_SERVER_Data_t *pData)
{
  ...
  {
    APP_DBG_MSG("  Fail: aci_gatt_update_char_value_ext command, error code: 0x%2X\n", ret);
    return ret;
  }
  else
  {
      ...
      switch(p_blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
        {
          ...
          else if(p_attribute_modified->Attr_Handle == (P2P_SERVER_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            ...
            notification.DataTransfered.Length = p_attribute_modified->Attr_Data_Length;
            notification.DataTransfered.p_Payload = p_attribute_modified->Attr_Data;
            notification.Offset = p_attribute_modified->Offset;
