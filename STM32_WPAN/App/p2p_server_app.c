...
uint8_t a_P2P_SERVER_UpdateCharData[512];
...
static void P2P_SERVER_Switch_c_SendNotification_Ext(void);
/* USER CODE END PFP */
...
void P2P_SERVER_Notification(P2P_SERVER_NotificationEvt_t *p_Notification)
{
  ...
  switch(p_Notification->EvtOpcode)
  {
    ...
    case P2P_SERVER_LED_C_WRITE_NO_RESP_EVT:
      ...
      if (p_Notification->DataTransfered.p_Payload[1] == 0x00 || p_Notification->DataTransfered.p_Payload[1] == 0x01) /* Write */
      {
        LOG_INFO_APP("-- P2P APPLICATION SERVER : WRITE RECEIVED\n");
        LOG_INFO_APP("** Write Data: 0x%02X %02X\n\n", p_Notification->DataTransfered.p_Payload[0], p_Notification->DataTransfered.p_Payload[1]);
      ...
      {
        LOG_INFO_APP("-- P2P APPLICATION SERVER : EXTENDED WRITE RECEIVED\n");
        LOG_INFO_APP("** Extended Write Data (Length: %d | Offset: %x): 0x", p_Notification->DataTransfered.Length, p_Notification->Offset);
        for (uint16_t i = 0; i < p_Notification->DataTransfered.Length; i++)
        {
          LOG_INFO_APP("%02X ", p_Notification->DataTransfered.p_Payload[i]);
        }
        LOG_INFO_APP("\n\n");
      }
...
void P2P_SERVER_APP_Init(void)
{
  ...
  UTIL_SEQ_RegTask( 1U << CFG_TASK_SEND_NOTIF_EXT_ID, UTIL_SEQ_RFU, P2P_SERVER_Switch_c_SendNotification_Ext);
...
void P2P_SERVER_Switch_c_SendNotification_Ext(void) /* Property Notification */
{
  P2P_SERVER_Data_t p2p_server_notification_data;

  p2p_server_notification_data.p_Payload = (uint8_t*)a_P2P_SERVER_UpdateCharData;
  p2p_server_notification_data.ExtendedLength = 509;
  
  for (uint16_t i = 0; i < p2p_server_notification_data.ExtendedLength; i++)
  {
    if (i < 254)
    {
      a_P2P_SERVER_UpdateCharData[i] = i + 1;
    }
    else
    {
      a_P2P_SERVER_UpdateCharData[i] = 0x0A;
    }
  }
  
  P2P_SERVER_UpdateValue_Ext(0x0, P2P_SERVER_SWITCH_C, &p2p_server_notification_data);
